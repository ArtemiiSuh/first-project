...
const (
	runCalMeanSpeedMult = 18   // множитель средней скорости.
	runCalSpeedShift    = 1.79 // среднее количество сжигаемых калорий при беге.
)
...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	run := ((runCalMeanSpeedMult * meanSpeed(action, duration) * runCalSpeedShift) * weight / mInKm * duration * minInH)
	return run
}

// Константы для расчета калорий, расходуемых при ходьбе.
const (
	walkCalWeightMult   = 0.035 // множитель массы тела.
	walkSpeedHeightMult = 0.029 // множитель роста.
)
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	walk := walkCalWeightMult * weight
	walks := math.Pow((meanSpeed(action, duration)*kmhInMsec), 2.0) / (height / cmInM) * walkSpeedHeightMult * weight
	gowalk := (walk + walks) * duration * minInH
	return gowalk
}
...
const (
	...
	swimCalWeightMult     = 2   // множитель веса при плавании.
)
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	swim := (swimmingMeanSpeed(lengthPool, countPool, duration) + swimCalMeanSpeedShift) * swimCalWeightMult * weight * duration
	return swim
}
